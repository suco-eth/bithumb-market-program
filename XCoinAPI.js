  ...
  xcoinApiCall(endPoint, params) {
    ...
    return new Promise(function (resolve, reject) {
      request(options, function (error, response, body) {
        if (!error && response.statusCode == 200 && JSON.parse(body).status === "0000") {
          resolve(response);
        } else {
          if (JSON.parse(body).status !== "5100") {
            console.log("error", JSON.parse(body));
            document.querySelector("#error").innerText = JSON.parse(body).message;
          }
          reject(error);
        }
      });
  ...
  _getHttpHeaders(endPoint, rgParams, api_key, api_secret) {
    ...
    return {
      ...
      "Api-Sign": base64_encode(
        CryptoJS.HmacSHA512(endPoint + chr(0) + strData + chr(0) + nNonce, api_secret).toString()
      ),
      "Api-Nonce": nNonce,
    };
  }
